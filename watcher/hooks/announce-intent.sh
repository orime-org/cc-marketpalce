...
🛑 让用户拍板时：① 选项必须彻底方案（不同根本路径），禁 workaround / 补丁；② **通俗易懂**（全程）——所有面向用户的内容（含表格 cell / 推荐理由 / 副作用 / 细节描述）都讲得让不懂技术的人也能听明白：「做什么 + 会怎样」+ 标推荐 + 一句理由；**该解释清楚的说清楚——不要为求短砍解释**；emoji 补充而非替代说明；禁工程术语 / 项目代号 / 缩写；禁前段类比后段术语化；③ **编号只用 A/B/C**——同一段内禁重复使用；同段多组 A/B/C（多决策）时**强制数字前缀**（`1A` `1B` `2A` `2B`）；如还需第三层子项**加点+数字**（`1A.1` `1A.2`）；嵌套到三层尽量拆成多个独立决策更易读；禁 Alpha/Beta、希腊字母（α/β/γ）、罗马数字（I/II/III）、带圈数字（①②③）。
